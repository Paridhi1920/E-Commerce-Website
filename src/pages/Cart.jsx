...
import { removeCart, increseQuantity, decreaseQuantity } from '../store/cartSlice'
        ...
        <div className='container mx-auto py-8 min-h-96 px-4 md:px-16 lg:px-24'>
            {cart.products.length > 0 ?
                <div>
                    ...
                    <div className='flex flex-col md:flex-row justify-between space-x-10 mt-8'>
                                    ...
                                    <div key={product.id} className='flex items-center justify-between p-3 border-b'>
                                        <div className='md:flex items-center space-x-4'>
                                            ...
                                            <div className='flex-1 ml-4'>
                                                <h3 className='lg:text-lg font-semibold md:text-l'>{product.name}</h3>
                                            </div>
                                        </div>
                                        <div className='md:text-l flex gap-2.5  lg:flex  items-center'>
                                            <p>₹{product.price}</p>
                                            <div className='flex items-center justify-center border '>
                                                <button className='text-xl font-bold px-1.5 border-r' onClick={() => dipatch(decreaseQuantity(product.id))}>-</button>
                                                <p className=' text-xl px-2'>{product.quantity}</p>
                                                <button className='text-xl  font-bold px-1.5 border-l' onClick={() => dipatch(increseQuantity(product.id))}>+</button>
                                            </div>
                                            <p> ₹{(product.quantity * product.price.toFixed(2))}</p>
                                            <button className='text-red-500 hover:text-red-700'
                                                onClick={() => dipatch(removeCart(product.id))}>
                                                <FaTrashAlt />
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className='w-1/3 bg-white p-6 rounded-lg shadow-md border'>
                            ...
                            <div className='justify-between mb-5 border-b pb-1'>
                ...
                : <div className='flex justify-center'>
                    <img src={empty} alt="No product" className='h-96' />
                </div>}
        </div>
    )
}
